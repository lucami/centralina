		...
		if( (temp-now)/CLOCKS_PER_SEC > 1)
        	{
            		...
            		//printf("H: %f\n",h/1024.0);
            		//printf("P: %d.%d\n",(p/256)/100, (p/256)%100);
            		data_len=sprintf(data_to_send, "%d.%d;%f;%d.%d",t/100,t%100,h/1024.0, (p/256)/100, (p/256)%100);
        	}

        	if(recvfrom(sockfd, (char *)buffer, MAXLINE,  0, ( struct sockaddr *) &cliaddr, &cli_len)>0)
        	{
            		sendto(sockfd, (const char *)data_to_send, data_len,  0, (const struct sockaddr *) &cliaddr, cli_len);
        	}
